...
| **Stats** | Open an interactive activity dashboard over the running sim. Pick a **time horizon** (1 min / 5 min / 15 min / all time / custom seconds) and a **metric**: per-kind *count* (total & bonded) or *actions/minute* (tick any set of **kinds** to overlay); a *chain-length histogram* (a live snapshot — all chains vs. those with an `End` — overlaid); or *chains over time* (number of chains, number complete with an `End`, and average length). A **chain** is a connected run of ≥2 `Chain`-kind balls (lone free monomers don't count); length is its ball count. Hover the chart for a tooltip reading off the datapoint under the cursor — the time and each series' value on the line plots, the length and its all/with-End counts on the histogram. |
